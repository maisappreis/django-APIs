...
class GenerateUploadURL(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            return Response({"error": f"Erro ao gerar URL de upload: {str(e)}"}, status=500)
...
#     """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not file_name:
            return Response({"error": "Nome do arquivo é obrigatório"}, status=400)

        # Converte altura se fornecida
        try:
            altura_cm = float(altura_cm) if altura_cm not in [None, "", "0"] else None
        except ValueError:
            altura_cm = None
        ...
        # Salva temporariamente para usar numpy-stl
        with tempfile.NamedTemporaryFile(delete=False, suffix=".stl") as tmp:
        ...
        try:
            ...
            largura_final_mm = largura_atual_mm * fator_escala
            comprimento_final_mm = comprimento_atual_mm * fator_escala
            altura_final_mm = altura_desejada_mm

            densidade = 1.24  # g/cm³
            volume_cm3 = volume_escalado_mm3 / 1000
            peso_g = volume_cm3 * densidade

            preco = peso_g * 1.5
            preco_rounded = self.arredondar_preco(preco)

            return Response({
                "altura_desejada_cm": round(altura_final_mm / 10, 2),
                "fator_escala": round(fator_escala, 3),
                "dimensoes_mm": {
                    "largura": round(largura_final_mm, 2),
                    "comprimento": round(comprimento_final_mm, 2),
